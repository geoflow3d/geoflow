...
void ImGradient::getTexture(unsigned char* tex)
{
    for(int i=0; i<256*3; i++){
        tex[i] = m_cachedValues[i] * 255;
    }
}
...
namespace ImGui
{
                                ...
                                float height)
    {
        ...
        // draw_list->AddRectFilled(ImVec2(bar_pos.x - 1, bar_pos.y - 1),
        //                          ImVec2(bar_pos.x + maxWidth + 1, barBottom + 1),
                                  ...
                                  float height)
    {
        ...
        for(auto markIt = gradient->getMarks().begin(); markIt != gradient->getMarks().end(); ++markIt )
        {
            ...
            draw_list->AddRectFilled(ImVec2(to - 4, bar_pos.y + (height + 2)),
                                               ImVec2(to + 4, bar_pos.y + (height + 10)),
                                               colorBU32);
    ...
    bool GradientEditor(const char* label, ImGradient* gradient,
                        ImGradientMark* & draggingMark,
                        ImGradientMark* & selectedMark, ImVec2 bar_size)
    {
        if(!gradient) return false;

        if(bar_size.y==0)
            bar_size.y = GRADIENT_BAR_EDITOR_HEIGHT;
        float maxWidth;
        if(bar_size.x==0)
            maxWidth = ImGui::GetContentRegionAvailWidth();
        else
            maxWidth = bar_size.x;
        ...
        if(selectedMark)
        {
            // bool colorModified = ImGui::ColorEdit3("color",selectedMark->color);
